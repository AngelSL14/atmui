...
class retiroOptionComponent extends Component {
    ...
    async componentWillMount() {
        ...
        if (s === null || s === undefined) { history("", this.props.history); }
            ...
            this.setState({ capabilities: s.capabilities, typeScreen: s.typeScreen, buttonHover: s.buttonHover });
    ...
    async handleLogicComponent() {
        ...
        if (montoMinimo['code'] !== undefined) {
            if (montoMinimo.code.includes("-") || montoMinimo.code === "404") {
                errorWS(montoMinimo);
                console.log(montoMinimo.code);
                history("retireTarjetaComponent", this.props.history);
            } else {
                this.setState({
                    montoMinimoMultiplo: montoMinimo.body[0].denomination
                });
    ...
    render() {
        const m = (
            <div>
                ...
                {!this.state.errorMonto && <h2 id="montoMensage">Hasta $8000 y múltiplos de {this.state.montoMinimoMultiplo}</h2>}
            </div>
        );
        return (
