...
class AvisoComisionComponent extends Component {
    ...
    componentWillMount() {
        ...
        if (ac === null || ac === undefined) { history("", this.props.history); }
            ...
            this.setState({ capabilities: ac.capabilities, typeScreen: ac.typeScreen, buttonHover: ac.buttonHover });
            setTimeout(() => { this.handleLogicComponent(); }, 40);
        }
        if (this.props.comision === "00.00") {
            this.setState({
                isComision: false
            })
    ...
    render() {
        let m = this.state.isComision ? (
            <div>
                <h1>¡Aviso!</h1>
                <LogoComision />
                <h2>Por esta operación pagará $<span>{this.props.comision} </span>  por comisión con IVA incluido</h2>
            </div>
        ) : (<div></div>);
        return (
