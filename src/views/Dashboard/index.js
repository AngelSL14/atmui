...
import { setDataFDK } from '../../Lib/utils/construction';

class Dashboard extends Component {

    stringBase = "WFS_PIN_FK_FDK0$1"
    ...
    strReplace = () => {
        let data = [];
        const strSplit = this.state.FDKMap.split("");
        strSplit.forEach((element, index) => {
            data[index] = this.stringBase.replace("$1", element)
        });
        return data;
    }
    ...
    handleStartInit = () => {
        try {
            ...
            this.handleReConnectService(error);
        }
    }
                    ...
                    if (jsonValidator(msg.data)) {
                        ...
                        if (json['hostName'] !== undefined) {
                            this.setState({
                                ...
                                FDKMap: json.fdkOrder,
                                entryMode: json.entryMode
                            });
                            setDataFDK(this.strReplace());
                        }
                    }
                }
            ...
            return (
                <Router>
                    <div className="dashboard" style={this.state.GDS}>
                        ...
                        <Route exact path="/" render={(props) => <Index {...props} StyleButtons={SB} hostName={this.state.hostName} ip={this.state.ip} track={this.state.track} modifyState={this.modifyState} token={this.state.token} entryMode={this.state.entryMode} />} />
